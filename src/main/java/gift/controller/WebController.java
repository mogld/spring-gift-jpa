...
public class WebController {
    ...
    public String showUserProductsPage(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, Model model) {
    ...
    @GetMapping("/products")
    public String viewProductPage(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, Model model) {
        Pageable pageable = PageRequest.of(page, size);
        ResponseEntity<Page<Product>> response = productController.getAllProducts(pageable);
        if (response.getStatusCode() != HttpStatus.OK || response.getBody() == null) {
            return "error/500";
